...
public class BookDAO {
	...
	/*
	 * Mapper xml을 실행시키기 위해서  SqlSessionTemplate객체를 멤버 변수로 선언한다.
	 * @Inject 어노테이션을 붙여서 SqlSessionTemplate객체를 사용할 수 있게 한다.
	 */
	@Inject
	...
	/*
	 * sqlSessionTemplate의 selectOne메소드는 데이터를 한 개만 가져올 때 사용한다.
	 ...
	 * commit은 따로 필요 없음. spring이 알아서 해줌.
	 */
	...
	public int updateBook(Map<String, Object> map) {
		/*
		 * sqlSessionTemplate 객체의 updateBook 메소드는 insert 메소드와 사용법이 동일하다
		 * 첫번째 파라미터는 쿼리ID, 두번째 파라미터는 쿼리 파라미터이고 반환값은 영향받은 행 수 이다.
		 */
	...
	public int removeBook(Map<String, Object> map) {
		/*
		 * sqlSessionTemplate 객체의 deleteBook 메소드는 update메소드와 사용법이 동일하다.
		 * 첫번째 파라미터는 쿼리ID, 두번째 파라미터는 쿼리 파라미터이고 반환값은 영향받은 행 수 이다.
		 */
	...
	public List<Map<String, Object>> selectBookList(Map<String, Object> map) {
		 ...
		 * 리턴타입을 List 타입으로 설정한 건, selectList 메소드의 결과가 집합 목록을 반환하기 때문이다.
		 */
